...
class CustomTokenizer:
    ...
    def _json_to_txt(self):

        data_dir = os.path.join(self.dir_path, self.prefix)
        if not os.path.isfile(os.path.join(data_dir, "train.json")):
            logger.info("Download benchmark dataset")
            parse_data(self.args)

        logger.info("Flatten Corpus to text file")
        with open(os.path.join(data_dir, "train.json")) as reader:
            lines = json.load(reader)
            self.data = []
            for line in lines:              
                self.data.append(''.join(line['text']))
    ...
    def train(self):
        ...
        #     self._ner_to_txt()
        # else:
        if self.prefix == 'bioasq':
            self._json_to_txt()
        elif self.prefix != 'pretrain':
            self._jsonl_to_txt()
