...
from Adapter import Roberta_Net_cls
...
def main():
    ...
    # Labels
    num_labels = max(raw_datasets["train"]["label"]) + 1
    ...
    metrics = [evaluate.load("accuracy"), evaluate.load("f1"), evaluate.load("f1")]

    # You can define your custom compute_metrics function. It takes an `EvalPrediction` object (a namedtuple with a
    # predictions and label_ids field) and has to return a dictionary string to float.
    def compute_metrics(p: EvalPrediction,label):
        results = []
        preds = p.predictions[0] if isinstance(p.predictions, tuple) else p.predictions
        preds = np.argmax(preds, axis=1)
        results.append(evaluate.load("accuracy").compute(predictions=preds, references=label))
        results.append(evaluate.load("f1").compute(predictions=preds, references=label, average = 'micro'))
        results.append(evaluate.load("f1").compute(predictions=preds, references=label, average = 'macro'))
    ...
    if training_args.do_predict:
        ...
        for predict_dataset, task in zip(predict_datasets, tasks):
            ori_label = predict_dataset['label']
            predict_dataset = predict_dataset.remove_columns("label")
            predictions, results = trainer.compute_metrics(trainer.predict(predict_dataset, metric_key_prefix="predict"),ori_label)
            predict_dataset = predict_dataset.add_column("label",ori_label)
